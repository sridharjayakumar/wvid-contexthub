...
(function($) {
    ...
    TestModule.prototype.defaultConfig = {
        ...
        /* store mappings */
        storeMapping: {
            myStore: 'dnb-persisted-store'
        },

        /* module template */
        template:
            '<p><b>{{i18n "D&B Visitor Info"}}</b></p>' +
            '<p>Company Name: {{myStore.companyName}}</p>' +
            '<p>DUNS Number: {{myStore.duns}}</p>' +
            '<p>NAICS Code: {{myStore.industryNAICS}}</p>' +
            '<p>Ultimate Parent: {{myStore.ultimateDUNS}}</p>' + 
            '<p>Number of Employees: {{myStore.employeesInAllLocations}}</p>'
    };
